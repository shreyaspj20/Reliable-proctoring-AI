...
import time
import numpy as np
from video_processing.face_detection import detect_faces
...
def register_user(fr, num_pics=5, skipr=False):  # Here model is Face_recogntion model
    ...
    while count < num_pics:
        ...
        if ret:
            ...
            cv2.putText(frame, 'Press r to capture image, {}/{} captures done'.format(count, num_pics), (30, 60),
...
def print_faces(frame, faces):
    for face in faces:
        ...
        # Face Spoofing
        if face.spoof != None:
            cv2.putText(frame, "real", (x + w + 5, y + 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0),
                        2) if face.spoof else cv2.putText(frame, "spoof", (x + w + 5, y + 10), cv2.FONT_HERSHEY_PLAIN,
                                                          1, (0, 0, 255), 2)
            cv2.putText(frame, "spf:" + str(round(face.spoof_score, 4)), (x + w + 5, y + 64), cv2.FONT_HERSHEY_PLAIN, 1,
                        (153, 0, 0), 1)
        ...
        # Mouth tracker
        if face.mouth:
            cv2.putText(frame, "MAR:" + str(round(face.mouth.mar, 4)), (x + w + 5, y + 82), cv2.FONT_HERSHEY_PLAIN, 0.9,
                        (153, 0, 0), 1)
